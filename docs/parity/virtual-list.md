...
A native desktop composition using GPUI's pinned `uniform_list` engine and Base GPUI's shared scrollbar. It is not a direct shadcn component port. Its typography, spacing, selected/hover colors, focus ring and radius use the same gpuicn theme as Sidebar and other controls.
